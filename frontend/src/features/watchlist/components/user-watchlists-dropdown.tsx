...
import { TvDetails, TvData } from '@/features/tv/types/tv-types'
...
const UserWatchlistsDropdown = ({
    userWatchlists,
    movie,
    tv,
}: UserWatchlistsDropdownProps) => {
    ...
    const addToWatchlist = async ({
        ...
        const response = await axiosPrivate.post(
            ...
            { id: itemId, media_type: mediaType(), status: 'queued', tags: [] }
        )
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        e.preventDefault()
        try {
            const response = await axiosPrivate.post('/watchlists/create', {
        ...
        <div className="text-sm md:text-base">
            {!userWatchlists.length ? (
                ...
                userWatchlists.map((watchlist) => (
                    <DropdownMenuItem
                        className="text-sm md:text-base max-w-[150px]"
                        key={watchlist.id}
