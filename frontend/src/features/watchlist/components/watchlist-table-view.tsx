...
import TagsPicker from '@/features/common/components/tags-picker'
...
const WatchlistTableView = ({
    ...
    const { userWatchlists, isUserWatchlistsLoading, refetchUserWatchlists } =
        useUserWatchlists()
        ...
        return watchlistItemsDetails.data
            .map((item) => {
                ...
                const matchingItem = watchlistDetails?.items.find(
                    (watchlistItem) =>
                        watchlistItem.id === mediaData.id &&
                        watchlistItem.media_type === type
                )
    ...
    const updateStatus = async ({
        ...
        return axiosPrivate.put(
            `/watchlists/${watchlistDetails?.id}/item/status`,
            {
                id,
                media_type,
                status,
            ...
            columnHelper.accessor('status', {
                ...
                cell: ({ row }) => {
                    const [selectedStatus, setSelectedStatus] =
                        useState<StatusType>(row.original.status)
                        ...
                        <div className="w-full flex justify-end">
                            <DropdownMenu>
                                ...
                                >
                                    <Button
                                        variant={'default'}
                                        className="p-0 w-6 h-6 rounded-full bg-background shadow"
                                    >
                                        <Ellipsis
    ...
    if (
        !watchlistItemsDetails.pending &&
        watchlistItemsDetails.data.length === 0
    ) {
        return (
            <div className="flex flex-col items-center justify-center py-12 text-center space-y-8">
                <div>
                    ...
                    <p className="text-foreground">
                        Add movies or TV shows to your watchlist to see them
                        here.
                    </p>
                </div>
