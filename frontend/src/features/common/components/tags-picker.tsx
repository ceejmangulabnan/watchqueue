...
import { TWatchlistItem } from '@/features/watchlist/components/watchlist-table-view'
import { Row } from '@tanstack/react-table'
import { WatchlistData, WatchlistItem } from '@/features/watchlist/types/watchlist-types'
import {
    DropdownMenu,
    DropdownMenuContent,
    DropdownMenuItem,
...
const TagsPicker = ({ row, watchlistDetails }: StatusPickerProps) => {
    ...
    const updateTags = (newTags: string[]) => {
        return axiosPrivate.put(
            `/watchlists/${watchlistDetails?.id}/tags`,
            newTags
        )
    }
    ...
    const updateItemTags = (updateWatchlistItemTags: WatchlistItem) => {
        return axiosPrivate.put(
            `/watchlists/${watchlistDetails?.id}/item/tags`,
            updateWatchlistItemTags
        )
    }
    ...
    const filteredTags = watchlistDetails?.all_tags.filter(
        ...
        <div className="flex items-center wrap gap-2">
            <div className="flex items-center gap-2">
                {selectedTags.map((tag) => (
                    <div
                        key={tag}
                        className="cursor-pointer flex items-center p-2 bg-gray-300/40 dark:dark:bg-gray-300/10 text-foreground hover:scale-105 transition-all ease-in-out rounded-md gap-2"
                    >
