...
import { TvDetails, RecommendedTvData } from '@/features/tv/types/tv-types'
...
const RecommendedTv = ({ tvDetails }: RecommendedTvProps) => {
    const fetchRecommendedTv = async () => {
        const response = await axiosBase.get(
            `/tv/${tvDetails.id}/recommendations`
        )
        return response.data as RecommendedTvData
    }

    const { data: recommendedTv } = useQuery({
        queryKey: ['recommendedTv', tvDetails.id],
        queryFn: fetchRecommendedTv,
    })
