...
const LandingPage = () => {
    ...
    const { data: tvTopRated, isLoading: isTvTopRatedLoading } = useMediaData(
        '/tv/top_rated',
        'tvTopRated'
    )
        ...
        <div className="mx-10 md:mx-20 my-10">
            <div className="mx-auto xl:max-w-[1400px] 2xl:max-w-[1600px]">
                ...
                {moviePopular && !isMoviePopularLoading && (
                    <ScrollableList
                        scrollableItems={moviePopular}
                        isDataLoading={isMoviePopularLoading}
                    />
                )}
                ...
                {movieTopRated && !isMovieTopRatedLoading && (
                    <ScrollableList
                        scrollableItems={movieTopRated}
                        isDataLoading={isMovieTopRatedLoading}
                    />
                )}
                ...
                {tvPopular && !isTvPopularLoading && (
                    <ScrollableList
                        scrollableItems={tvPopular}
                        isDataLoading={isTvPopularLoading}
                    />
                )}
                ...
                {tvTopRated && !isTvTopRatedLoading && (
                    <ScrollableList
                        scrollableItems={tvTopRated}
                        isDataLoading={isTvTopRatedLoading}
                    />
                )}
            </div>
        </div>
